		...
		$(document).ready(
			...
			function()
			{
				...
				pret = function(formats, front, verso, paper_weight, copies_number, folds_number)
				{
					...
					_printing_time = ( (papersheet_copies + parseInt(ground.papersheet_starting)) / ground.velocity ), // estimated number of hours required to print sheets on one side
					cuts = Math.floor(formats / 2) * Math.ceil(parseInt(papersheet_copies / 200)) * 0.25, // price for cutting sheets to final dimensions

					printing_price = (_printing_time < 1)
					? 1 * ground.printing_hour_price // under one hour considering one hour because of preparation of printing press 
					: 1 * ground.printing_hour_price + ((papersheet_copies - 1000) / ground.velocity) * ground.printing_hour_price, // estimated printing time price

					pf = (front == 0) ? 0 : printing_price, // face one printing price
					pv = (verso == 0) ? 0 : _printing_time*ground.printing_hour_price, // verso printing price
